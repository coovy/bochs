...
/* 为方便实现,块大小==扇区size */
   uint32_t boot_sector_sects = 1;	  // 引导块占用1扇区
   uint32_t super_block_sects = 1;    // 超级块占用1扇区

   /* inode位图占用扇区数 */
   uint32_t inode_bitmap_sects = DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR);	   // I结点位图占用的扇区数。最多支持4096个文件
   
   /* inode表占用扇区数 */
   uint32_t inode_table_sects = DIV_ROUND_UP(((sizeof(struct inode) * MAX_FILES_PER_PART)), SECTOR_SIZE);   // 最后一个扇区末尾有多余的位
...
空闲块位图，inode数组位图，可能占用几百扇区，不能用栈存储
*/
 ...
 */
   ...
   /* inode_table中共4096个inode => inode_bitmap占用1扇区,
...
/*
4 将inode数组初始化写入sb.inode_table_lba
*/
...
/* 为目录分配一个块,用来写入目录.和.. */
   ...
   create_dir_entry(dirname, inode_no, FT_DIRECTORY, &new_dir_entry); // 创建目录项new_dir_entry
   memset(io_buf, 0, SECTOR_SIZE * 2);	 // 清空io_buf
...
/* 至此full_path_reverse中的路径是反着的, 即子目录在前(左),父目录在后(右), 现将full_path_reverse中的路径反置 */
...
int32_t sys_stat(const char* path, struct stat* buf) {
   ...
   int32_t ret = -1;	// 默认返回值
   struct path_search_record searched_record;
   memset(&searched_record, 0, sizeof(struct path_search_record));   // 必须要初始化或清0, 否则栈中信息不知道是什么
   int inode_no = search_file(path, &searched_record);
   if (inode_no != -1) {
