...
/* 将内存中bitmap第bit_idx位所在的512字节同步到硬盘, btmp_type代表块位图还是inode位图 */
void bitmap_sync(struct partition* part, uint32_t bit_idx, uint8_t btmp_type) {
   uint32_t off_sec = bit_idx / 4096;  // 本i结点索引相对于位图的扇区偏移量, 一个扇区4096位
...
创建inode: 向inode_bitmap申请获得inode号, 更新inode_table的某项用inode填充
向block_bitmap申请扇区存入inode->i_sectors
...
*/
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
   ...
   uint32_t* all_blocks = (uint32_t*)sys_malloc(BLOCK_SIZE + 48);	  // 12+128=140块， 140*4=560B,用来记录文件所有的块地址
   if (all_blocks == NULL) {
