...
/* 硬盘各寄存器的端口， 查看"records/硬盘控制器主要端口寄存器.png", p126 */
/* Prinmary的通道的话，base是0x1F0, Secondary是0x170*/
#define reg_data(channel)     (channel->port_base + 0) 
#define reg_error(channel)     (channel->port_base + 1)  // 写时是Feature(命令的参数)
#define reg_sect_cnt(channel)     (channel->port_base + 2) 
#define reg_lba_l(channel)     (channel->port_base + 3)
#define reg_lba_m(channel)     (channel->port_base + 4)
#define reg_lba_h(channel)     (channel->port_base + 5)
#define reg_dev(channel)     (channel->port_base + 6)
#define reg_status(channel)     (channel->port_base + 7)  // 写时Command
...
/* device寄存器的一些关键位 */
#define BIT_DEV_MBS    0xa0        // 1010 0000位7和位5固定为1，从位0计算
#define BIT_DEV_LBA    0x40      // 位6，LBA寻址模式
#define BIT_DEV_DEV    0x10      // 位4，LBA
//剩下三位LBA的23~27位

/* 一些硬盘操作的指令 */
#define CMD_IDENTIFY       0xec        // identify指令，获取硬盘身份信息
...
uint8_t channel_cnt;       // 按硬盘数计算的通道数，这样可能不是特别合理
...
struct partition_table_entry {
   uint8_t  bootable;         // 是否可引导, 0x08表示为活动分区    
   uint8_t  start_head;         // 起始磁头号
   uint8_t  start_sec;         // 起始扇区号
   uint8_t  start_chs;         // 起始柱面号
   uint8_t  fs_type;         // 分区类型, 分区文件系统标志, 0x05h,0x0Fh 表示扩展分区；0x06h表示FAT16分区；0x0Bh表示FAT32分区；0x07h表示NTFS 分区
...
} __attribute__ ((packed));     // 告诉gcc编译器不要进行对齐, 保证此结构是16字节大小

/* 引导扇区, mbr或ebr所在的扇区 */
struct boot_sector {
   uint8_t  other[446];         // 引导代码
   struct   partition_table_entry partition_table[4];       // 分区表中有4项, 共64字节
   uint16_t signature;         // 启动扇区的结束标志是0x55,0xaa, 小端字节，实际上是0xaa55
} __attribute__ ((packed));   // __attribute__ ((packed))告诉编译器不要使用内存对齐
...
/* 等30秒
在ata手册中有这么一句话：“ All actions required in this state shall be completed within 31 s”
 */
static bool busy_wait(struct disk* hd) {
   struct ide_channel* channel = hd->my_channel;
   uint16_t time_limit = 30 * 1000;         // 等30000毫秒
...
/* 1 先选择操作的硬盘 */
   ...
   /* 3 执行的命令写入reg_cmd寄存器 */
      cmd_out(hd->my_channel, CMD_READ_SECTOR);          // 硬盘开始准备数据
...
分区表扫描需要以MBR引导扇区为入口, 遍历所有主分区，然后找到总扩展分区，在其中递归遍历每一个子扩展分区，找到逻辑分区。
需要给每个分区起名，借鉴linux方案:
[x]d[y][n],x表示硬盘分类h(IDE), s(SCSI);
d固定表示disk;
y表示设备号, 区分第几个设备, 小写字符，a是第一个，b第二个......
n表示分区号,从1开始
 */
...
void intr_hd_handler(uint8_t irq_no) {
   ASSERT(irq_no == 0x2e || irq_no == 0x2f); // 硬盘的中断向量号，0x2e
   uint8_t ch_no = irq_no - 0x2e;      // 刚好得到通道索引
   struct ide_channel* channel = &channels[ch_no];
   ASSERT(channel->irq_no == irq_no);
/* 主动操作硬盘时申请了锁，所以不用担心无法分清中断信号来自同一通道哪块硬盘
