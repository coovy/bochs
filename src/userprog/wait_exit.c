...
static void release_prog_resource(struct task_struct* release_thread) {
    ...
    // user_pde_nr是pde的数量，刚好是用户内存空间
    uint16_t user_pde_nr = 768, pde_idx = 0;
    uint32_t pde = 0;
    uint32_t* v_pde_ptr = NULL;	    // v表示var, 和函数pde_ptr区分

    // user_pde_nr每个页表中的pte数量，
    ...
    /* 回收页表中用户空间的页框, 采用遍历页表发现p位为1则回收的方法。
    之前在虚拟内存池pcb->userprog_vaddr是检查位图为1的bit再逐位回收 */
    while (pde_idx < user_pde_nr) {
        v_pde_ptr = pgdir_vaddr + pde_idx;  // 回忆一下页表项结构，高20位是页地址，然后后面12位是修改访问读写权限p位这些
        pde = *v_pde_ptr;
    ...
    /* 回收用户虚拟地址池本身所占的物理内存*/
    uint32_t bitmap_pg_cnt = (release_thread->userprog_vaddr.vaddr_bitmap.btmp_bytes_len) / PG_SIZE;    // 位图占用的页面数量
...
/* list_traversal的回调函数, 查找其parent_pid是ppid并且状态为TASK_HANGING的任务 */
...
/* list_traversal的回调函数, 将一个子进程过继给init(简单修改一下parent_pid即可) */
...
/* 工作: 等待子进程调用exit, 将子进程的退出状态保存到status指向的变量。成功则返回子进程的pid, 失败则返回-1 */
