...
/* 32位elf头 */
struct Elf32_Ehdr {
   unsigned char e_ident[16]; // 4字节魔数(0x7f+ELF的ascii,0x45,0x4c,0x46)+1B的ELF文件类型+编码格式(LSM小端，MSB大端)+ELF头版本信息，7~15暂时不用
   Elf32_Half    e_type;   // 文件类型
   Elf32_Half    e_machine; // 目标文件体系结构(硬件平台)
   Elf32_Word    e_version;  // 版本信息
   Elf32_Addr    e_entry;  // 控制权转交的虚拟地址
   Elf32_Off     e_phoff;  // 程序头表再文件内的字节偏移量，没有程序头表为0
   Elf32_Off     e_shoff;  // 同上，但是节头表
   Elf32_Word    e_flags;  // 处理器相关标志
   Elf32_Half    e_ehsize; //elf header大小
   Elf32_Half    e_phentsize; // 程序头表每个条目大小
   Elf32_Half    e_phnum;  // 程序头表每个条目数量，即段数量
   Elf32_Half    e_shentsize; // 节大小
   Elf32_Half    e_shnum;  // 节数量
...
struct Elf32_Phdr {
   ...
   Elf32_Off  p_offset;    // 本段在文件内起始偏移字节
   Elf32_Addr p_vaddr;     // 本段在内存中的起始虚拟地址
   Elf32_Addr p_paddr;     // 保留，仅用于物理地址相关的系统中
   Elf32_Word p_filesz;    // 本段在文件中的大小
   Elf32_Word p_memsz;     // 本段在内存中的大小
   Elf32_Word p_flags;     // 读写权限读写执行->4 2 1, 其他与操作系统和处理器相关的
   Elf32_Word p_align;     // 对齐方式，0/1是不对其，否则p_align是2的倍数
};


/* 和我的ubuntu22.04中的usr/include/elf.h中对比了一下，这个Elf32_Ehdr和Elf32_Phdr定义确实一样，
不过segment_type好像只覆盖了基本段类型， linux中是宏定义的形式，有很多 */
...
/* 将文件描述符fd指向的文件中, 偏移为offset, 连续大小为filesz的段加载到虚拟地址为vaddr的内存 */
static bool segment_load(int32_t fd, uint32_t offset, uint32_t filesz, uint32_t vaddr) {
   uint32_t vaddr_first_page = vaddr & 0xfffff000;    // 页框地址
   ...
   while (page_idx < occupy_pages) {
      ...
      if (!(*pde & 0x00000001) || !(*pte & 0x00000001)) {      // 最低位为p位
...
static int32_t load(const char* pathname) {
   ...
   /* 校验elf头，除了前四字节以外，后面要求是32位，小端，版本1，可执行 */
   if (memcmp(elf_header.e_ident, "\177ELF\1\1\1", 7) || elf_header.e_type != 2 \
      || elf_header.e_machine != 3 || elf_header.e_version != 1 \
