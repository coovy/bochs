...
<font size="2">&emsp;关于内存管理，这是个很精妙的东西，以往在书上基本都只是知道由虚拟地址到物理地址转换那一套东西。在内核中，我们维护着一个内存池的结构体(主要包含内存大小，起始地址，位图，一个锁),由这个结构体实例化出**全局的内核物理内存池和用户物理内存池**。内核有自己的页表结构，这是以前书上的一套。然后每个进程都有自己的页表结构，存储虚拟地址到物理地址的映射。这样实现虚拟地址的连续和物理地址的不连续。</font>
